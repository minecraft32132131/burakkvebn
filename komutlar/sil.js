...
module.exports.run = async (client, message, args) => {
    if (!message.member.hasPermission("728999852314460171")) return message.channel.send(":no_entry: Bu komutu kullanabilmek için `Üyeleri Yasakla` yetkisine sahip olmanız gerek.");
    if (!args[0]) {
           ...
           .then(bans => {
               ...
               message.guild.ban(seyfooo, sebeb)
                   .then(async (member) => {
                       ...
                       message.channel.send(`<@!${user.id}> sunucudan yasaklandı <a:banned:704458602714234931>`);
                   })
                   .catch(error => {
